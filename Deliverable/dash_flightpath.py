...
FEATURE_COLS_fp = flight_path_df.columns
...
with tab2:
    ...
    biplot_df["mean_arrival_delay"] = flight_path_df["mean_arrival_delay"].values
    fig = px.scatter(biplot_df, x="PC1", y="PC2", opacity=0.35,
                      title="PCA Biplot: Flights (points) + Feature Loadings (arrows)")
    scale = 3 * biplot_df[["PC1", "PC2"]].abs().quantile(0.95).max()
    loadings_2d = pd.DataFrame(pca_model.components_[:2].T, index=FEATURE_COLS_fp, columns=["PC1", "PC2"])
    ...
    loadings = pd.DataFrame(pca_model.components_.T, index=FEATURE_COLS_fp, columns=pca_cols)
...
with tab3:
    ...
    plot_df2["Anomaly"] = flight_path_df[f"flag_{algo_pick}"].map({True: "Anomaly", False: "Normal"})
    plot_df2["Score"] = flight_path_df[f"score_{algo_pick}"]
    fig = px.scatter_3d(
    ...
    # plot_df3["Votes"] = flight_path_df["consensus_votes"]
    # if n_components >= 3:
    #     fig = px.scatter_3d(
